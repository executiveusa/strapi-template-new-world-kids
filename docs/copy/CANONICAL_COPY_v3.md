# NWKids — Canonical Copy v3 (FINAL — supersedes all prior versions)

**Render order:** Hero → Programs → Timeline → Trust → Support → Studio
**Approved by Bambu, July 2026. This is the authority. Components render exactly this.**

---

## SECTION 1 — HERO (`NonprofitHero.tsx`)

**Eyebrow:**
Proyecto Indigo Azul food forest

**Proverb (hero ONLY — do not repeat anywhere else on the page):**
"If you ever think you're too small to make a difference, try going to sleep with a mosquito in the room."
— West African Proverb

**Headline (3 lines, large serif):**
Most kids graduate
without ever learning
the most important life skills.

**Sub-headline (gold):**
We fix that. Free. No exceptions.

**Body:**
New World Kids is a Seattle-based nonprofit. We create projects and programs that teach inner-city and rural youth life skills. No matter the situation, we meet them where they are — then inspire them with the possibilities and skills to go beyond.

**Core four callout (NEW — one line, visible, gold):**
Our framework: Food · Water · Energy · Shelter. Every program teaches all four.

**CTAs:**
- Primary (coral, dominant): Plant a seed — give $25 →
- Secondary (small text link below): or see 5 years of work →

**Stat strip:**
- 200+ — plant varieties growing
- 1.5 — acres in Puerto Vallarta
- 5+ — years of operation
- $0 — cost to every student

**NOTE:** The INDIGO AZUL PROGRAM tag + blurb is REMOVED from the hero. It lives in the Programs section now (§2). The hero is brand-level — the umbrella over all programs.

---

## SECTION 2 — PROGRAMS (`ProgramsSection.tsx`) — NOW BEFORE TIMELINE

**Eyebrow:**
Two programs. Two communities. One mission.

**Headline:**
We meet youth where they are.

### Card 1 — Culture Shock Program

**Eyebrow:** Inner city youth · CULTURE SHOCK PROGRAM
**Title:** From survival mode to purpose.
**Body:**
After graduation most inner-city youth are thrown into an economy that values financial gain over creativity, and success over service. Without the right life skills, young people with raw talent and real potential get stuck in a desperate loop just trying to survive. Those pressures lead to poor decisions and the ever-present trap of crime, drugs, and destructive behavior.

Our Culture Shock program uses mentors with backgrounds in art, sports, and urban agriculture to help youth discover their full potential, find meaningful employment, avoid the traps and pitfalls of street life, and contribute their gifts to the world.

**Bullets:**
- Earn-while-you-learn stipends
- Mentors in art, sports, and urban agriculture
- International travel and cultural immersion
- Second language acquisition, job placement

**Status badge (NEW):** PILOT PROGRAM COMING 2027

**Link:** Learn more on our blog → (/blog)

### Card 2 — Proyecto Indigo Azul

**Eyebrow:** Rural youth · Proyecto Indigo Azul
**Title:** The ability to grow your own food changes everything.
**Body:**
The ability and knowledge of how to grow your food is a priceless yet undervalued and almost forgotten skill. Our Indigo Azul project is a 1.5-acre food forest demonstration site started in 2020. Based in the rural hills of Puerto Vallarta, Mexico, we have created sustainable and thoughtful solutions for what we call the core four: Food, Water, Energy, and Shelter. We are now ready to create certified training courses for youth interested in our programs. All knowledge and lessons learned will be documented on our blog. Human written, nature approved, and free forever.

Youth in rural areas face different challenges than youth in the city. Many families live day to day, markets are far away and expensive, and the basics — food, water, shelter — are never guaranteed. Knowing how to produce your own can be life-changing. We're now raising funds to get the site government-certified and launch our first pilot program.

**Bullets:**
- 200+ plant species, fruit trees, and medicinal herbs
- Techniques practiced and documented year-round
- Sustainable systems designed to be replicated anywhere
- Free for every student, no exceptions

**Link:** See the gallery → (/gallery)

**Bottom CTA:** Support both programs → (/donate)

**NOTE:** The third card (Media and AI Revenue Studio) is REMOVED from this section. Studio is its own section (§6) at the bottom.

---

## SECTION 3 — TIMELINE (`TimelineSection.tsx`) — NOW AFTER PROGRAMS

**Eyebrow:** A story in chapters
**Headline:** From bare soil to a living ecosystem
**Sub-heading:** 6 seasons of documented, continuous work. Every chapter is real.
**Summary line (above cards):** 4 of 6 chapters complete. We're in Chapter 5 now.

### Chapter 1 · 2020-2021
**Title:** The soil gets repaired first
**Tagline:** The beginning.
**Photo alt:** Field documentation from April 10, 2021
**Highlights:**
- 1.5 acres secured
- Soil restoration started
- First planting cycles completed
**Status:** ✓ Complete

### Chapter 2 · 2021-2022
**Title:** The community starts to return
**Tagline:** The idea became real when people began to return.
**Photo alt:** Community documentation from April 18, 2021
**Highlights:**
- Weekly planting sessions
- Youth participation increased
- Water systems piloted
**Status:** ✓ Complete

### Chapter 3 · 2023-2024
**Title:** The food forest takes root
**Tagline:** THE STORM.
**Body (expanded):** Biodiversity stops being a hope and becomes a pattern. On October 10, 2023, Hurricane Lidia hit Puerto Vallarta directly. We lost a significant number of trees and the fish pond. The food forest survived, adapted, and came back stronger — proof that regenerative systems can absorb what nature throws at them.
**Photo alt:** Food forest documentation from April 8, 2023
**Highlights:**
- 200+ plant varieties established
- Soilless agriculture put into practice
- Volunteer network engaged
- Hurricane Lidia recovery (Oct 2023)
**Status:** ✓ Complete

### Chapter 4 · 2024-2025
**Title:** Infrastructure begins for the school
**Tagline:** Trees and bamboo begin to reach harvest stage.
**Photo alt:** Infrastructure documentation from April 8, 2023
**Highlights:**
- Learning spaces
- Program structure formalized
- Trust reporting improved
**Status:** ✓ Complete

### Chapter 5 · 2025-2026
**Title:** Programs and operations concepts take shape
**Tagline:** The public work and the backend have to stay in sync.
**Photo alt:** Program documentation from May 1, 2023
**Highlights:**
- Active youth programs
- Operations systems active
- Public proof cadence live
**Status:** ● Active now (pulsing gold dot)

### Chapter 6 · 2027-2030
**Title:** Scale without losing the heart
**Tagline:** The model should grow without becoming abstract.
**Photo alt:** Scale planning documentation from May 1, 2023
**Highlights:**
- Land ownership path
- Program replication
- Long-horizon financial resilience
**Status:** ◌ Planned

**Mobile hint:** Swipe or use ← → to explore all chapters

---

## SECTION 4 — TRUST (`TrustSection.tsx`)

**Eyebrow:** 501(c)(3) Nonprofit · EIN 46-4779591
**Heading:** Verified & transparent.
**Sub-heading:** We build in public. Your donations help keep all our resources, solutions, and programs 100% free for every student who finds them.

**Verification grid (render as gold pill badges, not body text):**
- Fiscal sponsor → Humanitarian Social Innovations
- Legal structure → 501(c)(3) fiscally sponsored program
- Tax ID (EIN) → 46-4779591 — Verify at IRS Tax Exempt Search
- Indigo Azul site → Azucena 1112, Paso de Guayabo, Puerto Vallarta, MX 48373
- Seattle office → 6725 S 116th Pl, Seattle, WA 98178
- Contact → info@nwkids.org · Response within 48 hours

---

## SECTION 5 — SUPPORT (`SupportSection.tsx`)

**Heading:** Support the mission
**Sub-heading:** Choose one clear next step.

**NOTE:** The proverb is REMOVED from this section. It lives in the hero only.

**Donate tiers:**
- $25 — Plant a tree — at Proyecto Indigo Azul — Give $25 →
- $50 — Sponsor a child — for one month of programs — Most impactful — Give $50 →
- $100 — Fund language classes — for 4 weeks of instruction — Give $100 →

**Recurring note:** On the donate page, toggle "Make this monthly" to become a sustaining supporter.

**Secondary:**
- Volunteer → /work-with-us
- Instagram · Facebook · LinkedIn

---

## SECTION 6 — STUDIO (`StudioSection.tsx`)

**Eyebrow:** Mission-Funded Studio
**Heading:** We are not built to beg. We are built to produce.
**Body:**
Starting a nonprofit is overwhelming. Compliance, grant writing, fundraising, website design, social media — it crushes great ideas before they do any good. We know. We've been through all of it.

Over the last two years we built AI systems that drive our mission. We now offer them to other nonprofits at a discounted rate — because we understand your challenges. We live them.

**CTAs:**
- Work with the studio → (/work-with-us)
- See pricing → (/work-with-us#pricing)

**What we offer (eyebrow):** Available to nonprofits and social purpose companies at a discounted rate.
- AI agents that understand your mission
- Automated social media and content creation
- Grant support automation
- Mission dashboards and transparency infrastructure
- Full-stack website design
- Compliance reporting systems

**Closing CTA:** Start a conversation → (/work-with-us)

---

## CHANGES FROM v2 → v3 (what the builder changes)

1. Hero headline: "how to grow food" → "the most important life skills"
2. Hero body: updated grammar + "No matter the situation" line added
3. INDIGO AZUL PROGRAM block: REMOVED from hero, merged into Programs card 2
4. Core four callout: ADDED to hero (one gold line)
5. Section order: FLIPPED — Programs before Timeline (was Timeline first)
6. Programs: 3rd Studio card REMOVED (2 cards only)
7. Programs eyebrow: "Three programs" → "Two programs"
8. Chapter 1 tagline: added "The beginning."
9. Chapter 2 title: "The youth and community shows interest" → "The community starts to return"
10. Chapter 3: "Nature Attacks" → "THE STORM" + Hurricane Lidia (Oct 10 2023) added to body + highlights
11. Chapter 3 title typo: "takes root" (confirmed correct, no change)
12. Culture Shock: "PILOT PROGRAM COMING 2027" badge added
13. Culture Shock bullet: "Second language acquisition" → "Second language acquisition, job placement"
14. Indigo Azul card: expanded body (demonstration site + blog documentation line + fundraising line)
15. Stray fragment "Students graduate with a certificate in |altern" — DELETED
16. Proverb: removed from Support section (hero only)
17. Grammar: "ever present" → "ever-present", quotes removed from "Earn-while-you-learn", run-on sentences broken
18. Bottom CTA: "Support all three programs" → "Support both programs"

## NOTES FOR BUILDER

- You have volunteer sports footage to work with for Culture Shock visuals (Bambu confirmed). Use as card imagery / gallery when ready.
- The Hurricane Lidia addition (Ch3) is real history — Oct 10, 2023, direct hit on Puerto Vallarta. Trees and fish pond lost. Recovery is part of the story. This makes Chapter 3 honest and specific.
- "Human written, nature approved, and free forever" is a TAGLINE — treat it with weight. Consider it as the blog footer or a recurring brand line.
