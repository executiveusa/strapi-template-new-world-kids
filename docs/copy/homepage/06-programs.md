...
Source of truth for: `packages/shared-data` → `programCards` (consumed by `apps/ui/src/components/homepage/ProgramsSection.tsx`).

## Heading

- Eyebrow: Two programs. Two communities. One mission.
- Headline: We meet youth where they are.

## Bottom CTA

- Support both programs → (links to /donate)

## Program cards (rendered in this order)

### 1. Culture Shock Program
...
- The 3rd "Mission-Funded Studio" card is REMOVED from this section. The Studio lives in its own section (10) lower on the page. Putting it here created two competing CTAs at the same depth and buried the Studio pitch under youth-program content a client has no reason to read. (See Krug audit on competing CTAs.)
- Culture Shock title changed from "Culture Shock Program" to "From survival mode to purpose." — problem-first, not category-naming.
