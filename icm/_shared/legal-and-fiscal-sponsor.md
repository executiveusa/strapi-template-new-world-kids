...
No external grant submission is allowed unless **both** `hsi_review_status == approved` and `owner_approved == true`.

The review-status output presented to an agent or human must include both approval fields so the gate is machine-verifiable and cannot be bypassed by omission.
