    ...
    <section className="bg-white px-5 py-20 text-[var(--color-ink)] sm:px-8 md:px-10 md:py-28">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end lg:gap-16">
          ...
          <p className="max-w-2xl border-t border-black/15 pt-6 text-base leading-7 text-black/65 md:text-lg md:leading-8 lg:border-t-0 lg:border-l-2 lg:border-[var(--color-nwk-blue)] lg:pt-0 lg:pl-7">
            {t.body}
          </p>
        </div>
        ...
        {slots.length === 0 ? (
          ...
          <div className="mt-12 grid border-t border-l border-black/15 sm:grid-cols-2 lg:grid-cols-4">
            {slots.map((slot) => {
              const label =
                t.stages[slot.status as keyof typeof t.stages] ??
                slot.public_label ??
                slot.status
              return (
                <article
                  key={slot.slot_number}
                  className="min-h-40 border-r border-b border-black/15 p-5 sm:p-6"
                >
                  <div className="flex items-baseline justify-between gap-4">
                    <span className="text-4xl font-black tracking-[-0.06em] text-[var(--color-nwk-blue)]">
                      {String(slot.slot_number).padStart(2, "0")}
                    </span>
                    <span className="text-[10px] font-bold tracking-[0.14em] text-black/35 uppercase">
                      {slot.pathway
                        ? `${t.pathway} · ${slot.pathway.replaceAll("_", " ")}`
                        : t.open}
                    </span>
                  </div>
                  <p className="mt-8 text-lg leading-6 font-black tracking-[-0.025em]">
                    {label}
                  </p>
                </article>
              )
