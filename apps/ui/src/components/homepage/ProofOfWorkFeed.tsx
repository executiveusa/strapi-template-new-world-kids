      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-end lg:gap-16">
          <div>
            ...
            <h2 className="mt-4 max-w-5xl text-[clamp(3rem,9vw,6rem)] leading-[0.92] font-black tracking-[-0.055em] text-balance">{t.title}</h2>
          </div>
          <p className="max-w-2xl border-t border-black/15 pt-6 text-base leading-7 text-black/65 md:text-lg md:leading-8 lg:border-t-0 lg:border-l-2 lg:border-[var(--color-action-orange)] lg:pt-0 lg:pl-7">{t.body}</p>
        </div>
        ...
        {artifacts.length === 0 ? (
          ...
          <div className="mt-12 grid gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
            {artifacts.map((artifact, index) => (
              <figure key={artifact.id} className={index === 0 ? "sm:col-span-2 lg:col-span-2" : ""}>
                {artifact.asset_url && (
                ...
                <figcaption className="mt-4 border-t border-black/15 pt-4">
                  <div className="flex flex-wrap items-baseline justify-between gap-3">
                    <p className="text-sm font-black">{formatDate(artifact.captured_at, locale)}</p>
                    <p className="text-[10px] font-bold tracking-[0.14em] text-[var(--color-nwk-blue)] uppercase">{t.verified}</p>
                  </div>
                  {artifact.location && <p className="mt-2 text-sm text-black/50">{artifact.location}</p>}
                </figcaption>
              </figure>
            ))}
          </div>
        ...
        <Link href="/gallery" locale={locale} className="mt-10 inline-flex min-h-11 items-center text-sm font-black underline decoration-black/25 underline-offset-8 hover:decoration-black focus-visible:ring-2 focus-visible:ring-[var(--color-nwk-blue)] focus-visible:outline-none">{t.archive} →</Link>
      </div>
    </section>
  )
}
