...
export function SupportSection() {
  ...
  return (
    <section
      id="support"
      className="bg-[var(--color-ink)] px-5 py-20 text-white sm:px-8 md:px-10 md:py-28"
    >
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-4 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:gap-16">
          <h2 className="max-w-5xl text-[clamp(3rem,10vw,6.5rem)] leading-[0.9] font-black tracking-[-0.055em] text-balance">
            {t.title}
          </h2>
          <div className="border-t border-white/20 pt-6 lg:border-t-0 lg:border-l-2 lg:border-[var(--color-nwk-blue)] lg:pt-0 lg:pl-7">
            ...
            <Link
              href="/donate"
              locale={locale}
              className="mt-7 inline-flex min-h-12 w-full items-center justify-center rounded-full bg-white px-7 text-sm font-black text-[var(--color-ink)] transition-transform duration-200 hover:-translate-y-px focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-ink)] focus-visible:outline-none sm:w-auto"
            >
              {t.cta} →
            </Link>
          </div>
