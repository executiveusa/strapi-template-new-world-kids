    ...
    <main className="bg-[var(--color-bg)] text-[var(--color-text-primary)]">
      <section className="px-5 py-16 sm:px-8 sm:py-24 md:px-10 md:py-32">
        <div className="mx-auto max-w-7xl">
          ...
          <h1 className="mt-4 max-w-5xl text-[clamp(2.9rem,11vw,7rem)] leading-[0.93] font-black tracking-[-0.05em] text-balance sm:mt-5 sm:tracking-[-0.055em]">{t.title}</h1>
          <p className="mt-7 max-w-3xl border-t border-black/15 pt-6 text-[15px] leading-7 text-[var(--color-text-muted)] sm:mt-9 sm:pt-7 sm:text-lg sm:leading-8 md:text-xl">{t.body}</p>
        </div>
      </section>

      <section className="border-y border-black/15 px-5 py-14 sm:px-8 sm:py-16 md:px-10 md:py-20">
        <div className="mx-auto grid max-w-7xl gap-7 sm:gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-end lg:gap-16">
          <div>
            <p className="text-[10px] font-semibold tracking-[0.2em] text-[var(--color-nwk-blue)] uppercase sm:text-xs">{t.whyEyebrow}</p>
            <h2 className="mt-4 max-w-xl text-[clamp(2.05rem,8.5vw,4.8rem)] leading-[0.96] font-black tracking-[-0.045em] text-balance sm:tracking-[-0.05em]">{t.whyTitle}</h2>
          </div>
          <div className="border-t border-black/15 pt-6 lg:border-t-0 lg:pt-0">
            <p className="max-w-2xl text-[15px] leading-7 text-[var(--color-text-muted)] sm:text-lg sm:leading-8">{t.whyBody}</p>
            <a href={`${siteLinks.email}?subject=${encodeURIComponent(locale === "es" ? "Idea para la bitácora de New World Kids" : "New World Kids journal idea")}`} className="mt-7 inline-flex min-h-12 w-full items-center justify-between border-y border-black/20 py-3 text-sm font-black hover:border-black/55 sm:w-auto sm:min-w-64">{t.invite}</a>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 pb-20 sm:px-8 sm:pb-24 md:px-10 md:pb-32">
        <div className="mx-auto max-w-7xl">
          {journalPosts.map((post, index) => {
            ...
            return (
              <article key={post.slug} className="grid gap-5 border-b border-black/15 py-8 sm:gap-6 sm:py-10 md:grid-cols-[92px_1fr_auto] md:items-start md:gap-8 md:py-12">
                ...
                <div>
                  ...
                  <h2 className="mt-3 max-w-4xl text-[clamp(1.85rem,8vw,4rem)] leading-[0.98] font-black tracking-[-0.04em] text-balance sm:tracking-[-0.045em]">{article.title}</h2>
                  <p className="mt-4 max-w-2xl text-[15px] leading-7 text-[var(--color-text-muted)] sm:text-lg sm:leading-8">{article.dek}</p>
                </div>
                <Link
                  href={`/blog/${post.slug}`}
                  locale={locale}
                  className="group inline-flex min-h-12 w-full items-center justify-between border-y border-black/20 py-3 text-sm font-black md:w-auto md:min-w-28"
                >
                  <span>{t.read}</span>
