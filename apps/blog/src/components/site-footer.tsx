  ...
  return (
    <footer className="bg-background border-t">
      <div className="container mx-auto flex flex-col gap-8 px-6 py-8 lg:px-0">
        ...
        <div className="text-muted-foreground rounded-sm border px-4 py-3 text-center text-sm">
        ...
        <div className="flex flex-col items-center justify-center gap-4 text-center md:flex-row md:gap-6">
          <div className="text-muted-foreground text-sm">
            {t("copyright")}
            {" © "}
            {new Date().getFullYear()} <span className="text-foreground font-medium">{siteConfig.author}</span>
          </div>
          <span className="text-muted-foreground text-sm">
            {t("lastBuilt")} <FormattedDate date={buildTime} />
          </span>
        </div>
      </div>
    </footer>
