...
async function generatePostData(filePath: string): Promise<Post> {
  ...
  const source = await serialize(content, {
    ...
    mdxOptions: {
      remarkPlugins: [
        remarkGfm,
        [remarkGitHub, { repository: 'executiveusa/strapi-template-new-world-kids' }],
        remarkGemoji,
        remarkMath,
        remarkDirective,
        remarkAdmonitions,
...
async function getPostsData(locale: Locale = 'en'): Promise<Post[]> {
  const postsData: Post[] = []
  const contentPath = getContentPath(locale)

  // Check if locale directory exists, fallback to English if not
  try {
    await fs.access(contentPath)
  } catch {
    const fallbackPath = getContentPath('en')
...
async function getPostsMeta(locale: Locale = 'en', cachedData?: Post[]): Promise<Metadata> {
  const postsData = cachedData ?? (await getPostsData(locale))

  const posts = postsData.map((post) => {
    const { excerpt: _, rawContent: ___, source: __, ...postMeta } = post
    return postMeta
  })
...
async function getPostData(slug: string, locale: Locale = 'en', cachedData?: Post[]): Promise<Post | undefined> {
  const postsData = cachedData ?? (await getPostsData(locale))
  ...
  if (!postData && locale !== 'en') {
    const fallbackPostsData = await getPostsData('en')
