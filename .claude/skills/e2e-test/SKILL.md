...
>
> 1. **Database type and connection** — what database is used and the environment variable name for the connection string
> 2. **Full schema** — every table, its columns, types, and relationships
> 3. **Data flows per user action** — for each user-facing action, document exactly what records are created, updated, or deleted
...
4. **Take a screenshot** — save to a descriptive path under `e2e-screenshots/`
...
1. Query the database to verify records
   ...
   - **SQLite:** use `sqlite3` directly
   - **Other databases:** write a small ad hoc script, run it, then delete it
2. Verify:
